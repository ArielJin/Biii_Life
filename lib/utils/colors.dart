import 'package:flutter/material.dart';

// const appColorPrimary = Color(0xFF2F65B9);
const appColorPrimary = Color(0xFF10E4AF);

const appOrangeColor = Color(0xFFff6900);
const appGreenColor = Color(0xFF00d084);
const blueTickColor = Colors.blue;
const starColor = Color(0xFFFFC840);

//Section BG / page BG
const appLayoutBackground = Color(0xFFF8F9FA);
const appSectionBackground = Color(0xFFFFFFFF);

// Tertiary
const bodyWhite = Color(0xFF6F7F92);
const bodyDark = Color(0xFFF5F5F5);
const dividerColor = Color(0xFFF1F1F1);

// Dark Theme Colors
// const appBackgroundColorDark = Color(0xFF121212);
// const appBackgroundColorDark = Color(0xFF121212);
const appBackgroundColorDark = Color(0xFF091025);
const cardBackgroundBlackDark = Color(0xFF111661);
const colorPrimaryBlack = Color(0xFF131d25);
const appColorPrimaryDarkLight = Color(0xFFF9FAFF);
const iconColorPrimaryDark = Color(0xFF212121);
const iconColorSecondaryDark = Color(0xFFA8ABAD);
const appShadowColorDark = Color(0x1A3E3942);
const aa = Color(0xff10E4AF);
